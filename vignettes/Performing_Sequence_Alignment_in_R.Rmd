...
vignette: >
  %\VignetteIndexEntry{Performing_Sequence_Alignment_in_R}
...
Although MSA is not on the focus of the **orthGS** package, it is an important tool for phylogenetic reconstructions and orthology/paralogy analyses. Thus, in the current vignette we offer some recomendations to perform multiple alignments without leaving the R context.
...
To download and install Clustal Omega you can go to https://www.clustal.org/omega/ and follows the instructions. Nevertheless, if you have a linux based OS you may benefit of the suitable package manager. For instance, for macOS I use `Homebrew`, which can be installed via CLI:
